WITH campaign_flattened AS (
    SELECT
        ...
        LOAD_TIMESTAMP
    FROM {{ ref('campaign') }},
    LATERAL FLATTEN(input => JSON_DATA:campaigns_data) f
),
sorted_campaign AS (
    SELECT
        ...
        COALESCE(TRY_TO_DECIMAL(REGEXP_REPLACE(budget,'[$,]',''),18,2),0) AS budget,
        COALESCE(TRY_TO_DECIMAL(REGEXP_REPLACE(total_cost,'[$,]',''),18,2),0) AS total_cost,
        COALESCE(TRY_TO_DECIMAL(REGEXP_REPLACE(total_revenue,'[$,]',''),18,2),0) AS total_revenue,
