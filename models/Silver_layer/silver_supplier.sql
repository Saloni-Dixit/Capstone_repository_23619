WITH supplier_flattened AS (
    SELECT
        ...
        LOAD_TIMESTAMP
    FROM {{ ref('supplier') }},
    LATERAL FLATTEN(input => JSON_DATA:suppliers_data) f
),
sorted_suppliers AS (
    SELECT
        ...
        COALESCE(
            ...
            TRY_TO_DATE(last_order_date,'MM-DD-YYYY'),
            TRY_TO_DATE(last_order_date,'DD-MM-YYYY'),
            TRY_TO_DATE(last_order_date,'MM/DD/YYYY'),
            TRY_TO_DATE(last_order_date,'DD/MM/YYYY')
        ) AS last_order_date,
        COALESCE(
        ...
        COALESCE(
            TRY_TO_DATE(contract_start_date,'YYYY-MM-DD'),
            TRY_TO_DATE(contract_start_date,'MM-DD-YYYY'),
            TRY_TO_DATE(contract_start_date,'DD-MM-YYYY'),
            TRY_TO_DATE(contract_start_date,'MM/DD/YYYY'),
            TRY_TO_DATE(contract_start_date,'DD/MM/YYYY')
        ) AS contract_start_date,
        COALESCE(
            TRY_TO_DATE(contract_end_date,'YYYY-MM-DD'),
            TRY_TO_DATE(contract_end_date,'MM-DD-YYYY'),
            TRY_TO_DATE(contract_end_date,'DD-MM-YYYY'),
            TRY_TO_DATE(contract_end_date,'MM/DD/YYYY'),
            TRY_TO_DATE(contract_end_date,'DD/MM/YYYY')
...
QUALIFY ROW_NUMBER() OVER(
    ...
    ORDER BY last_modified_date DESC, LOAD_TIMESTAMP DESC
)=1