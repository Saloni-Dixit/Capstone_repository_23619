WITH order_flattened AS (
    ...
    SELECT
        ...
        f.value:discount_amount::NUMBER(18,4) AS discount_amount,
        ...
        i.value:discount_amount::NUMBER(18,4) AS item_discount_amount,

        SOURCE_FILE,
        LOAD_TIMESTAMP

    FROM {{ ref('orders') }},
         LATERAL FLATTEN(input => JSON_DATA:orders_data) f,
...
sorted_orders AS (
    ...
    SELECT
        ...
        COALESCE(
            ...
            TRY_TO_TIMESTAMP_TZ(order_date,'YYYY-MM-DD HH24:MI:SS'),
        ...
        COALESCE(
            ...
            TRY_TO_TIMESTAMP_TZ(created_at,'YYYY-MM-DD HH24:MI:SS'),
        ...
        COALESCE(
            ...
            TRY_TO_TIMESTAMP_TZ(shipping_date,'YYYY-MM-DD HH24:MI:SS'),
        ...
        COALESCE(
            ...
            TRY_TO_TIMESTAMP_TZ(estimated_delivery_date,'YYYY-MM-DD HH24:MI:SS'),
        ...
        COALESCE(
            ...
            TRY_TO_TIMESTAMP_TZ(delivery_date,'YYYY-MM-DD HH24:MI:SS'),
        ...
        TRIM(REGEXP_REPLACE(billing_street,'[^A-Za-z0-9 ,.-]','')) AS billing_street,
        INITCAP(TRIM(billing_city)) AS billing_city,
        UPPER(TRIM(billing_state)) AS billing_state,
        TRIM(billing_zip) AS billing_zip,

        TRIM(REGEXP_REPLACE(shipping_street,'[^A-Za-z0-9 ,.-]','')) AS shipping_street,
...
SELECT
    ...
    (
        (m.line_revenue * (1 - s.discount_amount))
        - m.line_cost
        - s.shipping_cost
        - s.tax_amount
    ) AS profit_amount,
...
ORDER BY s.created_at DESC, s.LOAD_TIMESTAMP DESC
)=1